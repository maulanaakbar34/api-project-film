...
app.post('/auth/register', async (req, res, next) => {
    ...
    try {
        ...
        if (err.code === '23505') { // Kode error unik PostgreSQL untuk duplikasi (unique_violation)
...
// GET semua film dengan nama sutradara
...
// POST film baru (Membutuhkan autentikasi)
...
// PUT update film (Membutuhkan autentikasi dan role 'admin')
...
// DELETE film (Membutuhkan autentikasi dan role 'admin')
...
// POST sutradara baru (Membutuhkan autentikasi)
app.post('/directors', authenticateToken, async (req, res, next) => {
    ...
    try {
        ...
        if (err.code === '23505') { // Asumsi ada unique constraint pada nama sutradara
            return res.status(409).json({ error: 'Nama sutradara sudah ada' });
        }
        next(err);
    }
});

// PUT update sutradara (Membutuhkan autentikasi dan role 'admin')
...
// DELETE sutradara (Membutuhkan autentikasi dan role 'admin')
...
// Middleware penanganan error umum (500)
app.use((err, req, res, next) => {
    // Mencetak error ke console server untuk debugging
    console.error('[SERVER ERROR]', err.stack);
    // Mengirim respons error ke klien
